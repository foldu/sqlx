...
SELECT oid FROM pg_catalog.pg_type WHERE typname ILIKE $1
    ...
    pub(crate) async fn get_nullable_for_columns(
        ...
        let nullable_patch = self.nullables_from_explain(stmt_id, meta.parameters.len()).await?;
    ...
    async fn nullables_from_explain(&mut self, stmt_id: u32, params_len: usize) -> Result<Vec<Option<bool>>, Error> {
        ...
        let (Json([explain]),): (Json<[Explain; 1]>,)  = query_as(&explain)
            .fetch_one(self)
            .await?;
...
fn visit_plan(plan: &Plan, outputs: &[String], nullables: &mut Vec<Option<bool>>) {
    if let Some(plan_outputs) = &plan.output {
        ...
        if let Some("Full") | Some("Inner") = plan.join_type.as_deref().or(plan.parent_relation.as_deref()) {
